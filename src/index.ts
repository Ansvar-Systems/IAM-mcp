...
/**
 * IAM Expert MCP Server — stdio entry point.
 *
 * Provides IAM standards advisory, threat intelligence, protocol deep-dives,
 * architecture patterns, vendor configurations, and compliance cross-mapping
...
async function main() {
  const database = getDb();
  const capabilities = detectCapabilities(database);
  ...
  registerTools(server, database, capabilities, aboutContext);
